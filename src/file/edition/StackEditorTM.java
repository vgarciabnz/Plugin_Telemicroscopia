...
public class StackEditorTM implements PlugIn {
	ImagePlus imp;
	...
	void addSlice() {
		...
		imp.unlock();
		if (id != 0)
			IJ.selectWindow(id); // prevents macros from failing
	}
	...
	public void convertStackToImages(ImagePlus imp) {
		...
		for (int i = 1; i <= size; i++) {
			...
			String title = label != null && !label.equals("") ? label
					: getTitle(imp, i);
	...
	public Image[] getImages() {
		...
		ImageStack stack = imp.getStack();
		int size = stack.getSize();
		Image[] imagesArray = new Image[size];
		for (int i = 1; i <= size; i++) {
			ImageProcessor ip = stack.getProcessor(i);
			imagesArray[i - 1] = ip.createImage();
		}
		imp.unlock();
		return imagesArray;
	}

	void deleteSlice(int indice) {
		/*
		 * if (nSlices<2) {IJ.error("\"Delete Slice\" requires a stack");
		 * return;} if (imp.isDisplayedHyperStack()) {
		 * deleteHyperstackSliceOrFrame(); return; }
		 */
	...
	void addSlice(int indice) {
		// if (imp.isDisplayedHyperStack()) return;
		// Traemos la imagen de fichero
	...
	/*
	 * int size = stack.getSize(); if (size>30 && !IJ.isMacro()) { boolean ok =
	 * IJ.showMessageWithCancel("Convert to Images?",
	 * "Are you sure you want to convert this\nstack to "
	 * +size+" separate windows?"); if (!ok) {imp.unlock(); return;} }
	 * Calibration cal = imp.getCalibration(); CompositeImage cimg =
	 * imp.isComposite()?(CompositeImage)imp:null; if
	 * (imp.getNChannels()!=imp.getStackSize()) cimg = null; for (int i=1;
	 * i<=size; i++) { String label = stack.getShortSliceLabel(i); String title
	 * = label!=null&&!label.equals("")?label:getTitle(imp, i); ImageProcessor
	 * ip = stack.getProcessor(i); if (cimg!=null) { LUT lut =
	 * cimg.getChannelLut(i); if (lut!=null) { ip.setColorModel(lut);
	 * ip.setMinAndMax(lut.min, lut.max); } } ImagePlus imp2 = new
	 * ImagePlus(title, ip); imp2.setCalibration(cal); String info =
	 * stack.getSliceLabel(i); if (info!=null && !info.equals(label))
	 * imp2.setProperty("Info", info); imp2.show(); } imp.changes = false;
	 * ImageWindow win = imp.getWindow(); if (win!=null) win.close(); else if
	 * (Interpreter.isBatchMode()) Interpreter.removeBatchModeImage(imp);
	 * imp.unlock(); }
	 */

	String getTitle(ImagePlus imp, int n) {
		String digits = "00000000" + n;
		return imp.getShortTitle() + "-"
				+ digits.substring(digits.length() - 4, digits.length());
	}
